...
def setup_database(conn):
    try:
        cursor = conn.cursor()
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS "SalesByCountry" (
                "id" UUID PRIMARY KEY,
...
# Insert or update sales data by country
def insert_or_update_data(conn, rows):
    cursor = conn.cursor()
    query = '''
        INSERT INTO "SalesByCountry" ("id", "audioId", "ISRC", "name", "trackDownloads", "streams", "totalSales", "earnings")
    ...
    conn.commit()
    cursor.close()
    print(f"{len(rows)} sales by country rows upserted successfully.")
    ...
    if "data" in sales_data and sales_data["data"]:
        for entry in sales_data["data"]:
            transformed_data.append({
                ...
                "name": entry["country"],  # Use country as the name
